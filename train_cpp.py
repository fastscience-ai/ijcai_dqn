...
class CWrapper:
    ...
    def reset(self):
        ...
        self.env = test_handler.TestHandler(domain=self.domain, novelty=self.novelty, trial_novelty=self.novelty,
...
def main():
    ...
    # Setup test hande --> novelty (soo)r
    env = CWrapper(101, 'easy', seed=123) #200, 101
    ...
    if train:
        ...
        print("load model")
        dqn.load_weights('8.h5f')
        train_hist = dqn.fit(env, nb_steps=200000, visualize=False, verbose=2, callbacks=callbacks) #use 2 for recording
        dqn.save_weights('cartpole_200.h5f', overwrite=True)
        
    else:
        dqn.load_weights('cartpole_200.h5f')
    # Collect data samples for data plot
    r = dqn.test(env, nb_episodes=500, visualize=False)
    ave_reward = np.asarray(r.history['episode_reward']) / 200.0
    score = np.mean(np.asarray(r.history['episode_reward']) / 200.0)
    np.save("test_reward_101_"+str(score)+".npy",ave_reward)
